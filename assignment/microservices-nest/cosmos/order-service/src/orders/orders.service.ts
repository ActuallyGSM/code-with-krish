...
export class OrdersService implements OnModuleInit {
  ...
  private readonly consumer = this.kafka.consumer({ groupId: 'gihan-order-group' });
  ...
  async create(createOrderDto: createOrderDto): Promise<any> {
    ...
    for (const item of items) {
      const locKey = `gihanm-product:${item.productId}.lock`;
      const lock = await this.redis.set(locKey, new Date().toISOString(), 'EX', 3600 * 24, 'NX');
      if (!lock) {
        throw new BadRequestException(`Product ID ${item.productId} is beeing proceeded, Please try again later`,);
      }
  ...
  async updateOrderCity(id: number, city: string) {
    const order = await this.orderRepository.findOne({ where: { id } });
    if (!order) {
      throw new NotFoundException(`order with id: ${id} is not found`);
    }
  ...
  async consumeConfirmOrders() {
    await this.consumer.subscribe({ topic: 'gihanm-order.inventory.update' });
  }
}
