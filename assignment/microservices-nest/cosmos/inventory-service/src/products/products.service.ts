...
export class ProductsService implements OnModuleInit {
  ...
  private readonly consumer = this.kafka.consumer({ groupId: 'gihan-inventory-service' });
  ...
  async onModuleInit() {
    ...
    await this.consmerOrderCreated();
  }

  async consmerOrderCreated() {
    console.log('Consumer order created');
    await this.consumer.subscribe({ topic: 'gihanm2-order.create', fromBeginning: true });
    ...
    await this.consumer.run({
      eachMessage: async ({message }) => {
        ...
        // releaseing (deleting) the lock of redis
        for (const item of items) {
        ...
        await this.producer.send({
          topic: 'gihanm-order.inventory.update',
          messages: [
